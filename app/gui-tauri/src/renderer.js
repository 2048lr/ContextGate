...
async function loadBackground() {
  try {
    const url = await invoke('get_background_url')
...
async function loadStats() {
  try {
    const summary = await invoke('get_stats')
    if (summary) {
      stats = {
...
async function updateMemoryUsage() {
  try {
    const memData = await invoke('get_memory_usage')
    const heapUsed = Math.round((memData.heapUsed || 0) / 1024 / 1024)
    document.getElementById('memory-usage').textContent = `内存: ${heapUsed} MB`
  } catch (e) {
...
function setupEventListeners() {
  ...
  document.getElementById('btn-minimize').addEventListener('click', () => invoke('window_minimize'))
  document.getElementById('btn-maximize').addEventListener('click', () => invoke('window_maximize'))
...
async function selectProject() {
  try {
    ...
    if (folder) {
      ...
      await invoke('set_workspace', { workspace: folder })
      updateProjectUI()
    }
  } catch (e) {
...
async function buildContext() {
  ...
  try {
    const result = await invoke('build_context', { projectPath: currentProject })
    if (result.success) {
...
async function startProxy() {
  ...
  const result = await invoke('start_proxy', { port })
...
async function stopProxy() {
  const result = await invoke('stop_proxy')
  if (result.success) {
    proxyRunning = false
    updateProxyUI()
